...
<main id="main-content" class="grow lg:pt-0">
    <div class="page-heading pt-6 pb-10 text-center md:pt-12 md:pb-20">
        <div class="container">
            <h1 class="relative mb-3 text-3xl font-bold leading-tight tracking-tight md:text-5xl uppercase"> Nouveau mot de passe</h1>
            <ol
    ...
    <div class=" form-container col-start-2 col-end-12 py-10 md:col-end-6 md:py-20 max-w-3xl mx-auto">
        <div class="form-container bg-white border border-gray-200 p-5 rounded-md shadow-lg dark:bg-gray-800">
            <h2 class="font-bold text-2xl text-primary dark:text-white md:text-3.5xl mb-4 tracking-tight">Réinitialisation</h2>

            <div class="mb-8 md:mb-14 md:leading-normal md:text-lg tracking-tighter pr-2">
                <p>Entrez votre adresse e-mail et votre nouveau mot de passe pour réinitialiser vos informations.</p>
            </div>
<style>
         input,
            textarea,
...
@endif
            <form class="overflow-hidden flex flex-col gap-y-7" method="POST" action="{{ route('password.update') }}">
                @csrf
                <input class="text-base" type="hidden" name="token" value="{{ $token }}">

                <div class="">
                    <input
                        class="border-base text-base bg-white border rounded-md w-full px-7 py-4  focus:outline-0 focus:ring-0 dark:bg-gray-800 dark:text-white dark:border-white/10"
                        type="email"
                        name="email"
                        id="email"
                    ...
                    >
                    @error('email')
                        <p class="mt-2 text-sm font-bold text-danger">{{ $message }}</p>
                    @enderror
                </div>

                <div class="">
                    <input
                        class="text-base border-base bg-white border rounded-md w-full px-7 py-4 focus:outline-0 focus:ring-0 dark:bg-gray-800 dark:text-white dark:border-white/10"
                        type="password"
                        name="password"
                        id="password"
                ...
                <div class="">
                    <input
                        class="text-base border-base bg-white border rounded-lg w-full px-7 py-4 focus:outline-0 focus:ring-0 dark:bg-gray-800 dark:text-white dark:border-white/10"
                        type="password"
                ...
                <div class=" mt-3">
                    <input
                        class="text-base block w-full bg-[#ee1a3b] py-5 text-lg font-bold leading-normal tracking-tight text-white transition-colors hover:bg-[#ee1a3b]/90"
                        type="submit"
                        value="Réinitialisez le mot de passe"
                    >
                </div>
            </form>

            <div class=" pt-8">
                <div class="text-center font-bold text-primary mb-6 text-sm md:text-base dark:text-white">
                    Déjà un compte ? <a href="{{ route('login_page') }}" class="text-[#ee1a3b] hover:text-primary transition-colors">Connectez-vous ici</a>
                </div>
            </div>
        </div>
    </div>
    ...
    display: flex; /* Flex pour aligner les icônes */
    Centre horizontalement */
    width: 50px; /* Fixe la largeur du bouton */
    height: 50px; /* Fixe la hauteur du bouton */
    }

    /* Gère l'opacité et la visibilité des icônes */
    .material-icons {
    transition: opacity 0.1s ease, visibility 0.1s ease; /* Transition fluide */
    opacity: 0; /* Par défaut, invisible */
    visibility: hidden; /* Par défaut, caché */
    position: absolute; /* Positionne les icônes de manière absolue pour qu'elles ne poussent pas le bouton */
    }

    /* Icône visible */
    .material-icons.visible {
    opacity: 1; /* Affiche l'icône */
    visibility: visible; /* Rends l'icône visible */
    }

    /* Styles pour les icônes */
    .iconOpen, .iconClose {
    left: 0; /* Positionne l'icône de menu et de fermeture à gauche */
    margin-bottom: 5px; /* Ajuste la marge inférieure pour maintenir l'alignement */
    }
    ...
    .js-menu-toggle {
        display: none; /* Cache le menu burger sur les écrans larges */
    }
    }
      </style>
      <script>
