    ...
    <div class="container mt-4">
        <div class="row">
            @foreach ($advisories as $advisory)
                <div class="col-md-4">
                    <div class="card" advisory-id="{{ $advisory->id }}">
                        <!-- Affichage de l'image ou de la vidéo -->
                        @if ($advisory->isVideo)
                            <video class="card-img-top" controls>
                                <source src="{{ Storage::url($advisory->file) }}" type="video/mp4">
                                Votre navigateur ne supporte pas la lecture vidéo.
                            </video>
                        @else
                            <img src="{{ Storage::url($advisory->file) }}" class="card-img-top"
                                alt="Advisory #{{ $advisory->id }}">
                        @endif

                        <div class="card-body">
                            <!-- Infos principales -->
                            <p class="fs-6 mb-1 text-info">
                                Titre : <span class="advisory-title text-danger">{{ $advisory->title }}</span><br>
                                Position : <span
                                    class="advisory-position text-danger"><span class="d-none">{{ $advisory->position }}</span>{{ App\Enums\AdPosition::from($advisory->position)->label() }}</span><br>
        ...
        <!-- Offcanvas d'édition -->
        @include('backoffice.pages.advisory.formUpdate')

        @include('backoffice.pages.advisory.modalsConfirmDelete')
    </div>
@endsection
