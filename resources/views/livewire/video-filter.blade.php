    ...
    <div class="page-heading pt-6 pb-10 text-center md:pt-12 md:pb-20">
        <div class="container">
            <div class="h-auto object-contain p-6 w-full md:w-[85%] lg:w-[75%] xl:w-[65%] mx-auto mt-32 md:mt-100 mb-0 slide-container"
                id="slideContainer">
                @foreach ($advisoriesImagesUrl as $index => $url)
                    <div class="slide"
                        style="background-image: url('{{ $url }}'); transform: translateX({{ 100 * $index }}%)">
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <section class="bg-[#111827] pt-14 pb-14">
        ...
        <div class="container">
                ...
                <ul
                    ...
                    <li class="flex relative">
                        <a wire:click.prevent="setFilter('all')"
                            class="flex items-center py-2 font-bold transition-all hover:text-primary dark:hover:text-white hover:opacity-100 text-sm lg:text-base {{ $filter === 'all' ? 'opacity-100 text-primary border-b-3 border-[#ee1a3b]' : 'opacity-40' }} dark:text-white/90 cursor-pointer">
                            Toutes
                        </a>
                    </li>
                    <li class="flex">
                        <a wire:click.prevent="setFilter('recent')"
                            class="flex items-center py-2 font-bold transition-all hover:text-primary dark:hover:text-white hover:opacity-100 text-sm lg:text-base {{ $filter === 'recent' ? 'opacity-100 text-primary border-b-3 border-[#ee1a3b]' : 'opacity-40' }} dark:text-white/90 cursor-pointer">
                            Récentes
                        </a>
                    </li>
                    <li class="flex">
                        <a wire:click.prevent="setFilter('old')"
                            class="flex items-center py-2 font-bold transition-all hover:text-primary dark:hover:text-white hover:opacity-100 text-sm lg:text-base {{ $filter === 'old' ? 'opacity-100 text-primary border-b-3 border-[#ee1a3b]' : 'opacity-40' }} dark:text-white/90 cursor-pointer">
                            Anciennes
                        </a>
                    </li>
                    <li class="flex">
                        <a wire:click.prevent="setFilter('recommended')"
                            class="flex items-center py-2 font-bold transition-all hover:text-primary dark:hover:text-white hover:opacity-100 text-sm lg:text-base {{ $filter === 'recommended' ? 'opacity-100 text-primary border-b-3 border-[#ee1a3b]' : 'opacity-40' }} dark:text-white/90 cursor-pointer">
                            Recommandées
                        </a>
                    </li>
                    <li class="flex">
                        <a wire:click.prevent="setFilter('max_views')"
                            class="flex items-center py-2 font-bold transition-all hover:text-primary dark:hover:text-white hover:opacity-100 text-sm lg:text-base {{ $filter === 'max_views' ? 'opacity-100 text-primary border-b-3 border-[#ee1a3b]' : 'opacity-40' }} dark:text-white/90 cursor-pointer">
                            Plus vues
                        </a>
                    </li>
                </ul>
                    ...
                    <ul class="flex gap-2 pt-2 md:self-center md:pt-0">
                        <li>
                            <a wire:click.prevent="setGridCols(3)" class="cursor-pointer">
                                <svg role="img"
                                    class="h-6 w-6 {{ $gridCols === 3 ? 'fill-[#ee1a3b]' : 'dark:fill-white' }}">
                        ...
                        <li>
                            <a wire:click.prevent="setGridCols(4)" class="cursor-pointer">
                                <svg role="img"
                                    class="h-6 w-6 {{ $gridCols === 4 ? 'fill-[#ee1a3b]' : 'dark:fill-white' }}">
            ...
            <div class="grid grid-cols-12 gap-y-14 sm:gap-x-[30px] min-h-[500px] mt-[-100px]"
                ...
                @foreach (is_countable($videos) ? $videos : $videos->items() as $video)
                    <div
                        class="col-span-full sm:col-span-6 {{ $gridCols === 3 ? 'lg:col-span-4' : 'lg:col-span-3' }} video-item flex flex-col">
                        <figure class="relative mb-6">
                            <a class="group block h-full overflow-hidden bg-gray-900"
                                href="{{ route('video-watch', $video->slug) }}">
            ...
            {{-- Pagination --}}
            @if ($videos->count() > 0 && $videos->hasPages())
