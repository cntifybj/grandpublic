                ...
                <aside class="col-span-full md:col-span-4">
                    <div class="sticky lg:top-8 xl:top-10 bg-white dark:bg-gray-800 shadow-custom rounded-lg p-6 transition-smooth card-hover">
                        <div class="mb-6 text-center">
                            ...
                            <h2 class="mb-1 text-2xl font-bold text-primary dark:text-white">{{ Auth::user()->fullName() }}</h2>
                        </div>
                        <na class="py-1">
                     ...
                     <div id="info" class="section-content hhidden fade-in">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            <div>
                                ...
                                <p>{{ Auth::user()->fullName() }}</p> <!-- Affichage du nom de l'utilisateur -->
                            </div>
                            <div class="flex flex-col">
                                <p class="font-semibold mb-2">Email :</p>
                                <p>{{ Auth::user()->email }}</p> <!-- Affichage de l'email de l'utilisateur -->
                            </div>
                            ...
                            <div>
                                ...
                                <p>{{ \Carbon\Carbon::parse(Auth::user()->created_at)->translatedFormat('d F Y') }}</p>
                                <!-- Date d'inscription -->
                            </div>
                        </div>
                    </div>
                        ...
                        <div id="form" class="section-content hhidden fade-in">
                            ...
                            <form class="grid grid-cols-6 gap-x-7 gap-y-7 md:gap-x-7 lg:gap-x-[30px]" action="{{ route('profile.update') }}" method="POST">
                                @csrf
                                @method('PUT')
                                
                                <div class="col-span-full md:col-span-3">
                                    <label for="user-first-name" class="block mb-2 font-semibold">Nom </label>
                                    <input class="border border-gray-300 bg-white dark:bg-gray-800 text-primary dark:text-white rounded-md w-full px-4 py-3 focus:ring focus:ring-blue-500 focus:border-blue-500 transition duration-150 placeholder-gray-500" type="text" name="first_name" id="user-first-name" value="{{ Auth::user()->first_name }}" required>
                                </div>
                                <div class="col-span-full md:col-span-3">
                                    <label for="user-last-name" class="block mb-2 font-semibold">Prénom</label>
                                    <input class="border border-gray-300 bg-white dark:bg-gray-800 text-primary dark:text-white rounded-md w-full px-4 py-3 focus:ring focus:ring-blue-500 focus:border-blue-500 transition duration-150 placeholder-gray-500" type="text" name="last_name" id="user-last-name" value="{{ Auth::user()->last_name }}" required>
                                </div>

                                <div class="col-span-full md:col-span-3">
                                    <label for="user-email" class="block mb-2 font-semibold">Email</label>
                                    <input class="border border-gray-300 bg-white dark:bg-gray-800 text-primary dark:text-white rounded-md w-full px-4 py-3 focus:ring focus:ring-blue-500 focus:border-blue-500 transition duration-150 placeholder-gray-500" type="email" name="email" id="user-email" value="{{ Auth::user()->email }}" required>
                                </div>
